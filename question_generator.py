...
WORLDS = [
    ...
    {'id': 2, 'name': 'Multiplication Valley','levelStart': 6,'levelEnd': 10,'bossLevel': 10},
    {'id': 3, 'name': 'Fraction Kingdom','levelStart': 11,'levelEnd': 15,'bossLevel': 15},
    {'id': 4, 'name': 'Algebra City','levelStart': 16,'levelEnd': 20,'bossLevel': 20},
    {'id': 5, 'name': 'Geometry Jungle','levelStart': 21,'levelEnd': 25,'bossLevel': 25},
    {'id': 6, 'name': 'Coordinate Galaxy','levelStart': 26,'levelEnd': 30,'bossLevel': 30},
    {'id': 7, 'name': 'Trigonometry Tower','levelStart': 31,'levelEnd': 35,'bossLevel': 35},
    {'id': 8, 'name': 'Probability Lab','levelStart': 36,'levelEnd': 40,'bossLevel': 40},
    {'id': 9, 'name': 'Chaos Castle','levelStart': 41,'levelEnd': 45,'bossLevel': 45},
]

STORY_INTROS = [
    "Welcome, Explorer! A new puzzle awaits...",
    "You venture deeper into the unknown...",
    "A new challenge blocks your path!",
    "The crystal's glow grows stronger here...",
    "Your math skills are needed more than ever!",
]
...
def _arithmetic(diff):
    ...
    if diff >= 2 and random.random() > 0.5:
        ...
        return _mcq(f'{big} − {small} = ?', big - small,
                     f'Start at {big} and count back {small}.', '🧮 Arithmetic Puzzle')
    return _mcq(f'{a} + {b} = ?', a + b,
                f'Try counting up from {a} by {b} more.', '🧮 Arithmetic Puzzle')
...
def _multiplication(diff):
    ...
    if diff >= 3 and random.random() > 0.5:
        product = a * b
        return _mcq(f'{product} ÷ {a} = ?', b,
                     f'How many groups of {a} fit in {product}?', '➗ Division')
    return _mcq(f'{a} × {b} = ?', a * b,
                f'Think of {a} groups of {b}.', '✖️ Multiplication')
...
def _fractions(diff):
    if diff == 1:
        ...
        return _mcq(f'What fraction is {num} out of {denom} equal parts?',
                     f'{num}/{denom}',
                     f'If split into {denom} equal parts, and you have {num}...',
                     '🍕 Fraction Identification', is_text=True)
    elif diff == 2:
        d = random.choice([2, 3, 4, 6])
        n = random.randint(1, d - 1)
        m = random.randint(2, 4)
        return _mcq(f'Which fraction is equivalent to {n}/{d}?',
                     f'{n*m}/{d*m}',
...
def _algebra(diff):
    if diff == 1:
        ...
        return _mcq(f'Solve: x + {b} = {x + b}', x,
                     f'What number plus {b} equals {x + b}?', '🔤 Solve for x')
    elif diff == 2:
        x = random.randint(1, 10)
        a = random.randint(2, 5)
        return _mcq(f'Solve: {a}x = {a * x}', x,
                     f'Divide both sides by {a}.', '🔤 Equation Solving')
    else:
        ...
        return _mcq(f'Solve: {a}x + {b} = {result}', x,
                     f'First subtract {b}, then divide by {a}.', '🔤 Two-Step Equation')
...
def _coordinates(diff):
    if diff == 1:
        ...
        return _mcq(f'Find midpoint of ({x1},{y1}) and ({x2},{y2}). What is the x-coordinate?',
                    mx, 'Midpoint x = (x₁ + x₂) / 2', '📍 Midpoint', is_decimal=True)
    else:
        ...
        return _mcq(f'Distance from ({x1},{y1}) to ({x1+dx},{y1+dy})? (round to 1 decimal)',
                    dist, 'Use √((x₂−x₁)² + (y₂−y₁)²)', '📍 Distance', is_decimal=True)
...
def _trigonometry(diff):
    if diff == 1:
        sides = [
            ...
            ('The side opposite to angle θ', 'Opposite'),
            ('The side next to angle θ (not hypotenuse)', 'Adjacent'),
        ]
        q, a = random.choice(sides)
        return _mcq(f'In a right triangle, what is: "{q}"?', a,
                    'Think about the position relative to the angle.', '📐 Triangle Sides', is_text=True)
    elif diff == 2:
        defs = [
        ...
        q, a = random.choice(defs)
        return _mcq(q, a, 'Remember: SOH-CAH-TOA!', '📐 Trig Ratios', is_text=True)
    else:
        angles = [
        ...
        return _mcq(f'What is {f}({ang["deg"]}°)? (round to 2 decimals)',
                    ang[f], 'Remember the standard trig values!',
...
def _probability(diff):
    if diff == 1:
        ...
        return _mcq(f'Probability of rolling a {target} on a fair die? (as fraction)',
                    '1/6', f'Only 1 out of 6 outcomes is a {target}.',
